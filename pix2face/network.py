...
class Unet(nn.Module):
    def __init__(self, num_filters, channels_in=3, channels_out=3):
        ...
        self.batch_norm2_dn = nn.BatchNorm2d(num_filters * 2)
        self.batch_norm4_dn = nn.BatchNorm2d(num_filters * 4)
        self.batch_norm8_dn1 = nn.BatchNorm2d(num_filters * 8)
        self.batch_norm8_dn2 = nn.BatchNorm2d(num_filters * 8)
        self.batch_norm8_dn3 = nn.BatchNorm2d(num_filters * 8)
        self.batch_norm8_dn4 = nn.BatchNorm2d(num_filters * 8)
        self.batch_norm8_up1 = nn.BatchNorm2d(num_filters * 8)
        self.batch_norm8_up2 = nn.BatchNorm2d(num_filters * 8)
        self.batch_norm8_up3 = nn.BatchNorm2d(num_filters * 8)
        self.batch_norm8_up4 = nn.BatchNorm2d(num_filters * 8)
        self.batch_norm4_up = nn.BatchNorm2d(num_filters * 4)
        self.batch_norm2_up = nn.BatchNorm2d(num_filters * 2)
        self.batch_norm1_up = nn.BatchNorm2d(num_filters)
    ...
    def forward(self, input):
        ...
        e2 = self.batch_norm2_dn(self.conv2(self.leaky_relu(e1)))
        # state size is (num_filters x 2) x 64 x 64
        e3 = self.batch_norm4_dn(self.conv3(self.leaky_relu(e2)))
        # state size is (num_filters x 4) x 32 x 32
        e4 = self.batch_norm8_dn1(self.conv4(self.leaky_relu(e3)))
        # state size is (num_filters x 8) x 16 x 16
        e5 = self.batch_norm8_dn2(self.conv5(self.leaky_relu(e4)))
        # state size is (num_filters x 8) x 8 x 8
        e6 = self.batch_norm8_dn3(self.conv6(self.leaky_relu(e5)))
        # state size is (num_filters x 8) x 4 x 4
        e7 = self.batch_norm8_dn4(self.conv7(self.leaky_relu(e6)))
        ...
        d1_ = self.batch_norm8_up1(self.dconv1(self.up(self.relu(e8))))
        # state size is (num_filters x 8) x 2 x 2
        d1 = torch.cat((d1_, e7), 1)
        d2_ = self.batch_norm8_up2(self.dconv2(self.up(self.relu(d1))))
        # state size is (num_filters x 8) x 4 x 4
        d2 = torch.cat((d2_, e6), 1)
        d3_ = self.batch_norm8_up3(self.dconv3(self.up(self.relu(d2))))
        # state size is (num_filters x 8) x 8 x 8
        d3 = torch.cat((d3_, e5), 1)
        d4_ = self.batch_norm8_up4(self.dconv4(self.up(self.relu(d3))))
        # state size is (num_filters x 8) x 16 x 16
        d4 = torch.cat((d4_, e4), 1)
        d5_ = self.batch_norm4_up(self.dconv5(self.up(self.relu(d4))))
        # state size is (num_filters x 4) x 32 x 32
        d5 = torch.cat((d5_, e3), 1)
        d6_ = self.batch_norm2_up(self.dconv6(self.up(self.relu(d5))))
        # state size is (num_filters x 2) x 64 x 64
        d6 = torch.cat((d6_, e2), 1)
        d7_ = self.batch_norm1_up(self.dconv7(self.up(self.relu(d6))))
        # state size is (num_filters) x 128 x 128
